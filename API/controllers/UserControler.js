...
export const register = async (req, res) => {
    ...
    try {
        ...
        if (user) return res.status(404).json({ message: "this user allready exist", success: false })
        
        //   password hash  
         const hashpass = await bcrypt.hash(password,10)   
        user = await User.create({ name, email, password:hashpass });
        res.json({ massege: "user registration succes...!", success: true, user })
    } catch (err) {
...
export const login = async(req,res)=>{
       ...
       try{
        ...
        const token = jwt.sign({userId:user._id},'12121',{expiresIn:'220d'})
          res.json({message:`welcome Mr.${user.name} `,token,seccess:true})  
